...
const { state, saveCreds } = await useMultiFileAuthState('./src/sesi')
    ...
    tio.ev.on('groups.update', async tioxd => {
       ...
       tio.send5ButImg(tioxd[0].id, `Group telah ditutup oleh admin, sekarang hanya admin yang dapat mengirim pesan !`, `GROUP MESSAGE`, wm_tiodev, [])
       } else if(tioxd[0].announce == false) {
       tio.send5ButImg(tioxd[0].id, `Group telah dibuka oleh admin, sekarang peserta dapat mengirim pesan !`, `GROUP MESSAGE`, wm_tiodev, [])
       } else if (tioxd[0].restrict == true) {
       tio.send5ButImg(tioxd[0].id, `Info group telah dibatasi, sekarang hanya admin yang dapat mengedit info group !`, `GROUP MESSAGE`, wm_tiodev, [])
       } else if (tioxd[0].restrict == false) {
       tio.send5ButImg(tioxd[0].id, `Info group telah dibuka, sekarang peserta dapat mengedit info group !`, `GROUP MESSAGE`, wm_tiodev, [])  
       } else if (!tioxd[0].desc == '') {
       tio.send5ButImg(tioxd[0].id, `Group desk telah diganti menjadi ${tioxd[0].desc}*`, `GROUP MESSAGE`, wm_tiodev, [])
       } else if (!tioxd[0].icon_update == '') {
       tio.send5ButImg(tioxd[0].id, `Group icon telah diganti oleh admin`, `GROUP MESSAGE`, wm_tiodev, [])    
       } else if (!tioxd[0].inviteCode == '') {
       tio.send5ButImg(tioxd[0].id, `Group invite link telah di ubah\n\nNew invite link code: ${tioxd[0].inviteCode == 'undefined' ? '' : "https://chat.whatsapp.com/"+tioxd[0].inviteCode}`, `GROUP MESSAGE`, wm_tiodev, [])
       } else {
       tio.send5ButImg(tioxd[0].id, `Group Subject telah diganti menjadi ${tioxd[0].subject}`, `GROUP MESSAGE`, wm_tiodev, [])
  }
    })
...
tio.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: tekswell, footer: omlen })
//Message Saat Ada User Yang Keluar Dari Grup
} else if (anu.action == 'remove') {
teksbye = `Sayonara @${num.split('@')[0]}`
tio.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: teksbye, footer: omlen })
//Message Saat Ada Yang Naik Jabatan
} else if (anu.action == 'promote') {
tekspromote = `Congratulations @${num.split("@")[0]} you now admin in group ${metadata.subject}`
tio.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: tekspromote, footer: omlen })
//Message Saat Ada Yang Turun Jabatan
} else if (anu.action == 'demote') {
teksdemote = `@${num.split("@")[0]} sekarang bukan admin !`
tio.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: teksdemote, footer: omlen })
}
}
} else {
}
